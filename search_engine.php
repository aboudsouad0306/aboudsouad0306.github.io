...
if(isset ($_POST["civilite"]) and isset($_POST["nom"]) and isset($_POST["prenom"]) and isset($_POST["email"]) and isset($_POST["type"]) and isset($_POST["adresse"]) and isset($_POST["region"]))
{
	...
	$sql=$sql->query("SELECT * FROM wilaya WHERE wilaya='$region'");
	while($resultat=$sql->fetch())
	{
...
$sql=$bdd->query("SELECT * FROM entreprise WHERE nom LIKE '%$search%' OR raisonsocial LIKE '%$search%' OR description LIKE '%$search%'");

// j ai ajouter count okey

  //verification de la recherche
  //$nb_resultats=$sql->fetch();
  //if($nb_resultats!=0)
 // {
      ...
      while($resultat=$sql->fetch())
	     {
		  ...
		  echo ' <br> <a href="dzlocation/acceuil.php">Pour plus d infos voir DZLocation</a>'.'<br>';
		  echo '';
		  }
 // }//fin du if
  //else{
